...
@WebServlet("/Add_supplier_Servlet")
public class Add_supplier_Servlet extends HttpServlet {
	private static final long serialVersionUID = 1L;
	private Supplier_DAO supplierdao;
	HttpSession session;
	
    public Add_supplier_Servlet() {
        super();
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		session.setAttribute("session_status", supplierdao.addsupplier(supplier_info));
		response.sendRedirect("Redirect_Servlet?action=supplier");
	}
}