...
@WebServlet("/Add_Equipment_Type_Servlet")
public class Add_Equipment_Type_Servlet extends HttpServlet {
	private static final long serialVersionUID = 1L;
	private Equipment_Type_DAO equipmenttypedao;
	HttpSession session;
	
    public Add_Equipment_Type_Servlet() {
        super();
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		session.setAttribute("session_status", equipmenttypedao.addequipmenttype(equipment_type_info));
		response.sendRedirect("Redirect_Servlet?action=equipmenttype");
	}
}