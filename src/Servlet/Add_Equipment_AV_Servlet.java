...
@WebServlet("/Add_Equipment_AV_Servlet")
@MultipartConfig(
			fileSizeThreshold = 1024* 1024 * 10, //10Mb
			maxFileSize = 1024 * 1024 * 1000, //1Gb
			maxRequestSize = 1024 * 1024 * 1000 //1Gb
		)
public class Add_Equipment_AV_Servlet extends HttpServlet {
	private static final long serialVersionUID = 1L;
	private Equipment_DAO equipmentdao;
	HttpSession session;
	
    public Add_Equipment_AV_Servlet() {
        super();
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		session.setAttribute("session_status", equipmentdao.addequipmentav(equipment_av_info));
		response.sendRedirect("Redirect_Servlet?action=equipmentav");
	}
}