...
@WebServlet("/Update_Equipment_AC_Servlet")
@MultipartConfig(
		fileSizeThreshold = 1024* 1024 * 10, //10Mb
		maxFileSize = 1024 * 1024 * 1000, //1Gb
		maxRequestSize = 1024 * 1024 * 1000 //1Gb
	)
public class Update_Equipment_AC_Servlet extends HttpServlet {
	private static final long serialVersionUID = 1L;
	private Equipment_DAO equipmentdao;
	HttpSession session;
	
    public Update_Equipment_AC_Servlet() {
        super();
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(request.getParameter("equipment_quantity") != "") {
		...
		session.setAttribute("session_status", equipmentdao.updateequipmentac(equipment_ac_info));
		response.sendRedirect("Redirect_Servlet?action=equipmentac_update&id=" + request.getParameter("equipment_idnum"));
	}
}